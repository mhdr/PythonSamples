...
print("Starting update : {0}".format(time.asctime( time.localtime(time.time()) )))
for match in matches:
    match["LastName"]="Ramzani Sesemasi"
    id=match["_id"]
    people.update({"_id":id},{"$set":match},upsert=False)

print("End of update : {0}".format(time.asctime( time.localtime(time.time()) )))